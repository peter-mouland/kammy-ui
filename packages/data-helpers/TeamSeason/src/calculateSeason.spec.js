...
describe('calculateSeason.calculateSeasonPoints', () => {
  it('for a single gameweek', () => {
    const stats = {
      apps: 1, subs: 1, mom: 1, gls: 1, tb: 1, sb: 1, asts: 1, cs: 1, con: 1, pensv: 1, ycard: 1, rcard: 1,
    };
    const gameWeeks = [{ points: stats }];
    expect(calculateSeasonPoints(gameWeeks)).toEqual(stats);
  });
  ...
  it('for 2 gameweeks', () => {
    ...
    const gameWeeks = [{ points: stats }, { points: stats }];
    expect(calculateSeasonPoints(gameWeeks)).toEqual({
      apps: 2, subs: 2, mom: 2, gls: 2, tb: 2, sb: 2, asts: 2, cs: 2, con: 2, pensv: 2, ycard: 2, rcard: 2,
    });
  });
  ...
  it('for multiple gameweeks', () => {
    ...
    const gameWeeks = [{ points: stats }, { points: stats2 }, { points: stats3 }];
    expect(calculateSeasonPoints(gameWeeks)).toEqual({
      apps: 6, subs: 6, mom: 6, gls: 6, tb: 6, sb: 6, asts: 6, cs: 6, con: 6, pensv: 6, ycard: 6, rcard: 6,
    });
  });
